function reconstruct_L2_conformal(solution :: FEM.Solution_MsFEM,
    ...
    uOpt = reconstruct_L2_conformal(solution,
                                    mesh_collection,
                                    par,
                                    problem_f,
                                    u_orig,
...
function reconstruct_L2_conformal_old(solution :: FEM.Solution_MsFEM,
                                        mesh_collection :: Mesh.TriMesh_collection,
                                        par :: Parameter.Parameter_MsFEM,
                                        problem :: Problem.AbstractPhysicalProblem,
                                        problem_f :: Problem.AbstractBasisProblem,
...
function reconstruct_L2_conformal_old(solution :: FEM.Solution_MsFEM,
    ...
    system_matrix = [ (a_1^2 + k[4])*speye(n)   (a_1*a_2)*speye(n) ; 
                        (a_1*a_2)*speye(n)   (a_2^2 + k[4])*speye(n) ]

    rhs = ( [k[4]*basis_left + a_1*u_before ;
            k[4]*basis_right + a_2*u_before]
    ...
    ind_con = [sort(unique(m_f.segment[:,m_f.segment_marker.==1])) ; 
                sort(unique(m_f.segment[:,m_f.segment_marker.==2])) ; 
                sort(unique(m_f.segment[:,m_f.segment_marker.==3])) ; 
                sort(unique(m_f.segment[:,m_f.segment_marker.==1])) + n_dof ; 
                sort(unique(m_f.segment[:,m_f.segment_marker.==2])) + n_dof ; 
                sort(unique(m_f.segment[:,m_f.segment_marker.==3])) + n_dof ; 
                sort(unique(m_f.segment[:,m_f.segment_marker.==1])) + 2*n_dof ; 
                sort(unique(m_f.segment[:,m_f.segment_marker.==2])) + 2*n_dof ; 
                sort(unique(m_f.segment[:,m_f.segment_marker.==3])) + 2*n_dof ]
    # ind_con = [find(m_f.point_marker.!=0) ; find(m_f.point_marker.!=0)+n_dof ; find(m_f.point_marker.!=0)+2*n_dof]
    constr_val = uOpt[ind_con]
    ind_uncon = setdiff(1:(3*n_dof), ind_con)
