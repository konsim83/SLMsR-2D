function reconstruct_edge_L2_sumEqualOne!(uOpt :: Array{Float64,2},
	...
	# system_matrix = [ (a_1^2 + k_edge + k_sum)*speye(n)   (a_1*a_2 + k_sum)*speye(n) ; 
 #                        (a_1*a_2 + k_sum)*speye(n)   (a_2^2 + k_edge + k_sum)*speye(n) ]

 #    rhs = ( [k_edge*basis_left + k_sum*ones(n) + a_1*uOrigEdge ;
 #            k_edge*basis_right + k_sum*ones(n) + a_2*uOrigEdge]
