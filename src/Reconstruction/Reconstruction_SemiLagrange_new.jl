function SemiLagrange_L2_opt_new!(solution :: FEM.Solution_MsFEM,
	...
	if k_time==2
		display("\n---------------------------------------")
		display("Reconstruction of initial basis:")
		for i_mesh_local in 1:mesh_collection.mesh.n_cell
			u_basis_tmp = zeros(mesh_collection.mesh_f[i_mesh_local].n_point, 3)
			u_basis_tmp[:,:] = reconstruct_L2_conformal_new(solution,
															mesh_collection,
															dof_collection,
															ref_el_f,
			                        						quad_f,
	...
	if problem.conservative # || problem.reconstructEdge
		...
		display("Edge evolution --- yes")
	else
		display("---------------------------------------")
		display("Reconstruction at time step $(k_time) / $(par.n_steps+1):")
		display("Edge evolution --- no")
	end
	@time for i_mesh_local in 1:mesh_collection.mesh.n_cell
    	...
    	u_basis_tmp[:,:,1] = reconstruct_L2_conformal_new(solution,
															mesh_collection,
															dof_collection,
															ref_el_f,
			                        						quad_f,
