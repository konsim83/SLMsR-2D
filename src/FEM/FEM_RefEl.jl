...
function RefEl_Pk(n_order :: Int)
    ...
    return RefEl_Pk{n_order}(info_str, n_order, node, n_basis, coeff)
...
function shapeFun_grad(ref_el :: RefEl_Pk{1}, p :: Array{Float64})
    ...
    value = reshape([ref_el.coeff[i,1:2] for j=1:size(p,2) for i=1:ref_el.n_basis], ref_el.n_basis, size(p,2))
...
function shapeFun_grad(ref_el :: RefEl_Pk{2}, p :: Array{Float64})
    ...
    value = reshape([[dx[i] ; dy[i]] for i in eachindex(dx)], ref_el.n_basis, size(p,2))
