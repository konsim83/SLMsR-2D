...
function map_ref_point(mesh :: Mesh_1D, x :: Array{Float64,1}, ind_c :: Array{Int64,1})
	...
	P = [repeat(mesh.point[:,mesh.cell[1,ind]],1,length(x)) + [(mesh.point[1,mesh.cell[2,ind]]-mesh.point[1,mesh.cell[1,ind]])*transpose(x) ; 
...
function map_ref_point_der(mesh :: Mesh_1D, x :: Array{Float64,1}, ind_c :: Array{Int64,1})
	...
	P_der = sqrt.(sum((mesh.point[:,mesh.cell[2,ind_c]]-mesh.point[:,mesh.cell[1,ind_c]]).^2,dims=1))
